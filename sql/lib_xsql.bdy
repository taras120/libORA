create or replace package body lib_xsql is
  ...
  function get_xml_type(p_type# integer,
                        p_value in out varchar2) return varchar2 is
    ...
    begin
      ...
      if xs_types.count = 0 then
        ...
        if date_value = trunc(date_value) then
          ...
          result  := lib_xml.XS_DATE;
          p_value := lib_xml.xmlDate(date_value);
        
        else
        
          result  := lib_xml.XS_DATETIME;
          p_value := lib_xml.xmlDateTime(date_value);
        ...
        if length(to_char(number_value - trunc(number_value))) > 8 then
          result := lib_xml.XS_DECIMAL;
        else
          result := lib_xml.XS_DOUBLE;
        end if;
      
      else
        result := xs_types(1); -- default type
      end if;
  ...
  function get_xml_type(p_type# integer) return varchar2 is
    value varchar2(1);
  begin
  
    return get_xml_type(p_type#, value);
  end;
  ...
  begin
    ...
    rowset$ := lib_sql.fetch_as_rowset$(p_cursor => p_cursor,
                                        p_rows#  => p_rows#,
                                        b_close  => b_close);
    ...
    if p_rows# = 1 then
      ...
      while col is not null loop
        ...
        valueNode := lib_xml.createNode(rootNode, lib_text.lower_camel(col));
        lib_xml.setClob(valueNode, rec$(col));
      ...
      for n in 1 .. rowset$.count loop
        ...
        while col is not null loop
          ...
          valueNode := lib_xml.createNode(recNode, lib_text.lower_camel(col));
          lib_xml.setClob(valueNode, rec$(col));
  ...
  function get_value(p_node  dbms_xmldom.DOMNode,
    ...
    nodeText varchar2(32767);
  begin
  
    nodeText := lib_xml.getText(p_node);
  
    if p_type# is not null then
    
      if p_type# = lib_sql.PLS_BOOLEAN then
      
        return to_int(lib_xml.toBool(nodeText));
      
      elsif p_type# = lib_sql.ORA_DATE then
      
        return lib_xml.toDateTime(nodeText);
      
      elsif p_type# in (lib_sql.ORA_NUMBER, lib_sql.PLS_INTEGER) then
      
        return lib_xml.toNumber(nodeText);
  ...
  procedure serialize_result(p_node   dbms_xmldom.DOMNode,
                             p_result in out lib_sql.t_value) is
  begin
  ...
  -- result serializer
  function serialize_result(p_result in out lib_sql.t_value) return xmltype is
  ...
  function serialize_results(p_results in out lib_sql.t_values) return xmltype is
  ...
  function serialize_and_camelize(p_results in out lib_sql.t_values) return xmltype is
