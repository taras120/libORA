create or replace package body lib_xsql is
  ...
  begin
    ...
    -- XML compatible types
    XML_TYPE(lib_sql.PLS_INTEGER) := 'int';
    XML_TYPE(lib_sql.PLS_BOOLEAN) := 'boolean';
    XML_TYPE(lib_sql.ORA_VARCHAR2) := 'string';
    XML_TYPE(lib_sql.ORA_NUMBER) := 'decimal';
    XML_TYPE(lib_sql.ORA_DATE) := 'date';
    XML_TYPE(lib_sql.ORA_CHAR) := 'char';
    XML_TYPE(lib_sql.ORA_BINARY_FLOAT) := 'float';
    XML_TYPE(lib_sql.ORA_BINARY_DOUBLE) := 'double';
    XML_TYPE(lib_sql.ORA_CLOB) := 'clob';
    XML_TYPE(lib_sql.ORA_BLOB) := 'blob';
    XML_TYPE(lib_sql.ORA_TIMESTAMP) := 'timestamp';
    XML_TYPE(lib_sql.UDT_XMLTYPE) := 'xmltype';

    -- datatype reverse array
    type# := XML_TYPE.first;
    while type# is not null loop
      XML_TYPE#(XML_TYPE(type#)) := type#;
      type# := XML_TYPE.next(type#);
    end loop;
